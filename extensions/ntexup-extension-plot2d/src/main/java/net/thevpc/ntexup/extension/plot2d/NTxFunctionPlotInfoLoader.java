...
//    }
        ...
        if (nListContainerElement != null && !nListContainerElement.isAnyTuple()) {
            ...
            for (NElement child : c) {
                if (child.asObject().isPresent() && child.asObject().get().name().isPresent()) {
                    ...
                    buildContext.log(NMsg.ofC("unexpected plot child %s", NTxUtils.snippet(child)));
                }
            }
        }
        if (all.isEmpty()) {
    ...
    private NTxFunctionPlotInfo load(NObjectElement child, NTxNodeBuilderContext buildContext) {
        ...
        switch (NNameFormat.VAR_NAME.format(NStringUtils.firstNonNull(child.name().orNull(),"").trim())) {
            ...
            default: {
                buildContext.log(NMsg.ofC("unexpected plot type %s", child.name().orNull()));
                return null;
            }
        }
        for (NElement e : child.children()) {
            if (e.isNamedPair()) {
                switch (e.asPair().get().key().asStringValue().get()) {
                    ...
                    default: {
                        buildContext.log(NMsg.ofC("unexpected function declaration %s", NTxUtils.snippet(e)));
                    }
                }
            } else if (e.isPair()) {
                ...
                if (k.isNamedTuple("f")) {
                    ...
                    if (fk.params().size() == 1 && fk.params().get(0).isName()) {
                        ...
                        buildContext.log(NMsg.ofC("unexpected function declaration %s", NTxUtils.snippet(k)));
                    }
                } else {
                    buildContext.log(NMsg.ofC("unexpected function declaration %s", NTxUtils.snippet(k)));
                }
