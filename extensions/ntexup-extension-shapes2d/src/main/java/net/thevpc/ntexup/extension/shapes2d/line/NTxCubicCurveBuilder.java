...
public class NTxCubicCurveBuilder implements NTxNodeBuilder {
    ...
    public void renderMain(NTxNodeRendererContext rendererContext) {
        ...
        NTxPoint2D to = NTxPoint.ofParent(NTxValue.ofProp(node, NTxPropName.TO).asPoint2D().get()).valueHPoint2D(b, rendererContext.getGlobalBounds())
                .plus(translation);
        NTxPoint2D ctrl1 = NTxPoint.ofParent(NTxValue.ofProp(node, NTxPropName.CTRL1).asPoint2D().get()).valueHPoint2D(b, rendererContext.getGlobalBounds())
                .plus(translation);
        NTxPoint2D ctrl2 = NTxPoint.ofParent(NTxValue.ofProp(node, NTxPropName.CTRL2).asPoint2D().get()).valueHPoint2D(b, rendererContext.getGlobalBounds())
                .plus(translation);
