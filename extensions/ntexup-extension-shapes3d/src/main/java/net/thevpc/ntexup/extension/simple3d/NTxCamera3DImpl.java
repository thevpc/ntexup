...
public class NTxCamera3DImpl implements NTxCamera3D {
    ...
    @Override
    public NTxPoint2D projectFromLocalToScreen(NTxPoint3D localPoint, NTxMatrix3D worldTransform, NTxPoint2D screenOrigin) {
        if (worldTransform != null) {
            NTxPoint3D worldPoint = worldTransform.multiplyPoint(localPoint);
            return projectFromWorldToScreen(worldPoint, screenOrigin);
        }
        return projectFromWorldToScreen(localPoint, screenOrigin);
    }
