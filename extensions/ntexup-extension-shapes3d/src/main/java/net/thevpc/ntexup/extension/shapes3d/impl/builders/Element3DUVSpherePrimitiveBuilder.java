    ...
    public NtxElement3DPrimitive[] toPrimitives(NtxElement3D e, NTxRenderState3D renderState) {
        ...
        for (int j = 0; j < parallels - 1; ++j) {
            ...
            for (int i = 0; i < meridians; ++i) {
                ...
                double y = cp * radiusY + origin.y;
                double z = sp * sa * radiusZ + origin.z;
...
//            }
    ...
    public NtxElement3D createElement3D(NTxRendererContext rendererContext, NTxBounds2D b, RealToRelativeMapper mapper, NtxElement3DNodeParserFactory parserFactory) {
        ...
        NTxPoint3D radius = NtxShapes3dUtils.resolveDistance(node, "radius", rendererContext, b).orElse(NTxPoint3D.ofOne());
        int meridians = NTxValue.ofProp(node, "meridians").asInt().orElse(60);
        int parallels = NTxValue.ofProp(node, "parallels").asInt().orElse(60);
        NtxElement3D r = NTxElement3DFactory.sphereUV(position, radius.x, radius.y, radius.z, meridians, parallels);
