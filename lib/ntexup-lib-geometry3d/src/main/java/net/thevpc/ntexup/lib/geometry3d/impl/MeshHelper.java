...
public class MeshHelper {
    ...
    public static List<NtxElement3DTriangle> initialTriangulation(NtxElement3DPolygon poly) {
        ...
        for (int i = 1; i < pts.length - 1; i++) {
            ...
            tri.copyStyle(poly);
            tri.setContour(false);
            triangles.add(tri);
        }
        return triangles;
    }

    public static void triangulatePolygon(NtxElement3DPolygon p, List<? super NtxElement3DTriangle> out, double maxEdge) {
        for (NtxElement3DTriangle t : MeshHelper.initialTriangulation(p)) {
            MeshHelper.refineTriangle(t, out, maxEdge, p, false);
        }
    }

    public static boolean refineTriangle(NtxElement3DTriangle tri, List<? super NtxElement3DTriangle> out, double maxEdge, boolean showMesh) {
        return refineTriangle(tri,out,maxEdge,tri,showMesh);
    }

    public static boolean refineTriangle(NtxElement3DTriangle tri, List<? super NtxElement3DTriangle> out, double maxEdge, NtxElement3DPrimitive rootElement, boolean showMesh) {
        ...
        if (d01 > maxEdge || d12 > maxEdge || d20 > maxEdge) {
            ...
            NtxElement3DTriangle t1 = new NtxElement3DTriangle(p[0], m01, m20, fill, contour);
            t1.copyStyle(rootElement);
            if (!showMesh) {
                t1.setContour(false);
            }
            refineTriangle(t1, out, maxEdge, rootElement, showMesh);

            NtxElement3DTriangle t2 = new NtxElement3DTriangle(p[1], m12, m01, fill, contour);
            t2.copyStyle(rootElement);
            if (!showMesh) {
                t2.setContour(false);
            }
            refineTriangle(t2, out, maxEdge, rootElement, showMesh);

            NtxElement3DTriangle t3 = new NtxElement3DTriangle(p[2], m20, m12, fill, contour);
            t3.copyStyle(rootElement);
            if (!showMesh) {
                t3.setContour(false);
            }
            refineTriangle(t3, out, maxEdge, rootElement, showMesh);

            NtxElement3DTriangle t4 = new NtxElement3DTriangle(m01, m12, m20, fill, contour);
            t4.copyStyle(rootElement);
            if (!showMesh) {
                t4.setContour(false);
            }
            refineTriangle(t4, out, maxEdge, rootElement, showMesh);
